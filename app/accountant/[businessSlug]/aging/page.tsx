import { getAgingReport, getAccountantShops, exportAgingReport } from "../../actions"
import { requireAccountant } from "../../../../lib/auth"
import { AgingContent } from "./aging-content"
...
export default async function AccountantAgingPage({ params, searchParams }: Props) {
  const { businessSlug } = await params
  const searchParamsData = await searchParams
  const { membership } = await requireAccountant(businessSlug)
  
  const shops = await getAccountantShops(businessSlug)
  const agingReport = await getAgingReport(businessSlug, searchParamsData.shopId)
  ...
  return (
    <div className="p-8">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-400">Outstanding balances by age</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <AgingContent
        ...
        initialShopId={searchParamsData.shopId || ""}
      />
    </div>
  )
}
