          ...
          <>
            ...
            <div ref={printRef}>
              ...
              <div className="header text-center border-b-2 border-black pb-5 mb-5">
                <p className="shop-name text-2xl font-bold">{bill.shop.name}</p>
                ...
                <p className="bill-title text-xl font-bold mt-4 uppercase tracking-widest">
                  {bill.purchaseType === "CASH" ? "RECEIPT" : "INVOICE"}
                </p>
                <p className="bill-number text-gray-600">{bill.purchaseNumber}</p>
                <span className={`purchase-type ${bill.purchaseType} inline-block mt-2 px-3 py-1 rounded text-xs font-bold uppercase`}>
                  {bill.purchaseType}
                </span>
              </div>
              ...
              <div className="section info-grid grid grid-cols-2 gap-4 mb-5">
                <div>
                  <p className="section-title text-xs font-bold uppercase text-gray-500 border-b border-gray-200 pb-1 mb-2">Bill To</p>
                  <div className="info-item">
                    <label className="text-xs text-gray-500 uppercase">Customer Name</label>
                    <span className="text-sm font-medium">{bill.customer.name}</span>
                  </div>
                  <div className="info-item mt-2">
                    <label className="text-xs text-gray-500 uppercase">Phone</label>
                    <span className="text-sm font-medium">{bill.customer.phone}</span>
                  </div>
                ...
                <div className="text-right">
                  <p className="section-title text-xs font-bold uppercase text-gray-500 border-b border-gray-200 pb-1 mb-2">Details</p>
                  <div className="info-item">
                  ...
                  {bill.salesStaff && (
                    <div className="info-item mt-2">
                      <label className="text-xs text-gray-500 uppercase">Sales Staff</label>
                      <span className="text-sm font-medium">{bill.salesStaff}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="section mb-5">
                <p className="section-title text-xs font-bold uppercase text-gray-500 border-b border-gray-200 pb-1 mb-2">Items</p>
              ...
              {/* Totals */}
              <div className="totals border-t-2 border-black pt-4">
                ...
                {bill.interestAmount > 0 && (
                  <div className="total-row flex justify-between py-1">
                    <span>Interest ({bill.interestRate}% {bill.interestType})</span>
                    <span>{formatCurrency(bill.interestAmount)}</span>
                  </div>
                )}
                <div className="total-row highlight flex justify-between py-2 text-lg font-bold border-t border-gray-200 mt-2 pt-2">
                  <span>Total Amount</span>
                  <span>{formatCurrency(bill.totalAmount)}</span>
                </div>
                {bill.downPayment > 0 && (
                  <div className="total-row flex justify-between py-1 text-green-600">
                    <span>Down Payment</span>
                    <span>- {formatCurrency(bill.downPayment)}</span>
                  </div>
                )}
                {bill.outstandingBalance > 0 && (
                  <div className="total-row outstanding flex justify-between py-2 text-lg font-bold text-red-600">
                    <span>Balance Due</span>
                    <span>{formatCurrency(bill.outstandingBalance)}</span>
                  </div>
                )}
                {bill.outstandingBalance === 0 && (
                  <div className="total-row flex justify-between py-2 text-lg font-bold text-green-600">
                    <span>Status</span>
              ...
              {bill.purchaseType === "CREDIT" && bill.outstandingBalance > 0 && (
                <div className="payment-schedule mt-5 p-4 bg-gray-50 rounded-lg">
                  <h4 className="text-xs uppercase font-bold text-gray-600 mb-2">Payment Schedule</h4>
                  <p className="text-sm">Number of Installments: <strong>{bill.installments}</strong></p>
                  <p className="text-sm">Monthly Payment: <strong>{formatCurrency(Math.ceil(bill.outstandingBalance / bill.installments))}</strong></p>
                  <p className="text-sm">Due Date: <strong>{formatDate(bill.dueDate)}</strong></p>
                </div>
              )}

              {/* Footer */}
              <div className="footer mt-10 text-center text-xs text-gray-500 border-t border-gray-200 pt-5">
                <p>Thank you for your business!</p>
                <p>For inquiries, please contact us at {bill.shop.phone || "the shop"}</p>
