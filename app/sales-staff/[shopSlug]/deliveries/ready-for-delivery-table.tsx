...
import { ReadyForDeliveryData, updateDeliveryStatus } from "../../actions"
import { WaybillModal } from "./waybill-modal"

interface ReadyForDeliveryTableProps {
  deliveries: ReadyForDeliveryData[]
  shopSlug: string
}

export function ReadyForDeliveryTable({ deliveries, shopSlug }: ReadyForDeliveryTableProps) {
  ...
  const handleMarkDelivered = async (purchaseId: string) => {
    ...
    startTransition(async () => {
      const result = await updateDeliveryStatus(shopSlug, purchaseId, "DELIVERED")
      if (!result.success) {
  ...
  const handleStartDelivery = async (purchaseId: string) => {
    setProcessingId(purchaseId)
    startTransition(async () => {
      const result = await updateDeliveryStatus(shopSlug, purchaseId, "IN_TRANSIT")
      if (!result.success) {
