...
export function StaffReportsContent({ shopSlug, reports }: Props) {
  ...
  const selectedDateReports = useMemo(() => {
    ...
    return reports.filter(r => {
      ...
      return reportDateKey === selectedDate
    })
  ...
  const stats = useMemo(() => {
    ...
    const salesReports = reports.filter(r => r.reportType === "SALES").length
    const collectionReports = reports.filter(r => r.reportType === "COLLECTION").length
    return { pendingCount, totalSales, totalCollected, salesReports, collectionReports }
  }, [reports])
