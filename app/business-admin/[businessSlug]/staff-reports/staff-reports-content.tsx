...
import { reviewDailyReportAsBusinessAdmin, BusinessStaffDailyReportData } from "../../actions"
...
export function BusinessStaffReportsContent({ businessSlug, reports, shops }: Props) {
  const [shopFilter, setShopFilter] = useState<string>("all")
  const [typeFilter, setTypeFilter] = useState<"all" | "SALES" | "COLLECTION">("all")
  ...
  const totalSales = reports
    .filter((r) => r.reportType === "SALES")
    .reduce((sum, r) => sum + (r.totalSalesAmount || 0), 0)
  const totalCollected = reports
    .filter((r) => r.reportType === "COLLECTION")
    .reduce((sum, r) => sum + (r.totalCollected || 0), 0)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div className="glass-card p-5 rounded-2xl">
          <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">Total Sales Reported</p>
        ...
        <div className="glass-card p-5 rounded-2xl">
          ...
          <p className="text-2xl font-bold text-purple-400">₵{totalCollected.toLocaleString()}</p>
        </div>
        <div className="glass-card p-5 rounded-2xl">
          <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">Shops</p>
          <p className="text-2xl font-bold text-cyan-400">{shops.length}</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            value={typeFilter}
            onChange={(e) => setTypeFilter(e.target.value as "all" | "SALES" | "COLLECTION")}
            className="px-3 py-2 rounded-lg bg-slate-800 border border-white/10 text-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50 [&>option]:bg-slate-800 [&>option]:text-white"
          >
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-white">{selectedReport.shopName}</p>
                </div>
                <div>
