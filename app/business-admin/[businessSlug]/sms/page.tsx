...
import { Loader2, Send, MessageSquare, Users, History, AlertCircle, CheckCircle, Clock } from "lucide-react"
...
export default function BulkSmsPage() {
  ...
  async function loadData() {
    try {
      const [settingsData, customersData, logsData] = await Promise.all([
        getSmsSettings(businessSlug),
        getCustomersForSms(businessSlug),
        getSmsLogs(businessSlug, 1, 20),
      ])

      setSettings(settingsData)
      setCustomers(customersData.customers)
      setShops(customersData.shops)
  ...
  function getRecipientCount() {
    ...
    } else if (recipientType === "shop" && selectedShop) {
      return customers.filter((c) => c.shopId === selectedShop).length
  ...
  function selectAllCustomers() {
    const filteredCustomers = selectedShop
      ? customers.filter((c) => c.shopId === selectedShop)
      : customers
  ...
  async function handleSendSms() {
    ...
    try {
      ...
      if (recipientType === "shop" && selectedShop) {
        formData.set("shopId", selectedShop)
      }
      if (recipientType === "selected") {
        formData.set("selectedCustomerIds", JSON.stringify(selectedCustomers))
      }

      const result = await sendBulkSmsAction(businessSlug, formData)
  ...
  if (!settings || !settings.isEnabled) {
    return (
      <div className="flex flex-col items-center justify-center h-96 gap-4">
        ...
        <h2 className="text-xl font-semibold">SMS Not Configured</h2>
        <p className="text-muted-foreground text-center max-w-md">
          {!settings
            ? "Please configure your SMS settings before sending messages."
            : "SMS is currently disabled. Please enable it in settings."}
        </p>
        <Button asChild>
          <a href={`/business-admin/${businessSlug}/settings/sms`}>Configure SMS</a>
        </Button>
      </div>
    )
  }
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Select
                    value={recipientType}
                    onValueChange={(v) => {
                      setRecipientType(v as "all" | "shop" | "selected")
                      setSelectedCustomers([])
                    }}
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="shop">By Shop</SelectItem>
                      ...
                      <SelectContent>
                        {shops.map((shop) => (
                          <SelectItem key={shop.id} value={shop.id}>
                            {shop.name} (
                            {customers.filter((c) => c.shopId === shop.id).length} customers)
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
