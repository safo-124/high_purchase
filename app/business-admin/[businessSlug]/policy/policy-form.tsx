...
import { upsertBusinessPolicy, BusinessPolicyPayload, BusinessPolicyData } from "../../actions"
import { toast } from "sonner"

interface PolicyFormProps {
  businessSlug: string
  initialPolicy: BusinessPolicyData | null
}

export function PolicyForm({ businessSlug, initialPolicy }: PolicyFormProps) {
  const router = useRouter()
  const [isLoading, setIsLoading] = useState(false)
  
  const [interestType, setInterestType] = useState<"FLAT" | "MONTHLY">(initialPolicy?.interestType || "FLAT")
  const [interestRate, setInterestRate] = useState(initialPolicy?.interestRate?.toString() || "0")
  const [graceDays, setGraceDays] = useState(initialPolicy?.graceDays?.toString() || "3")
  const [maxTenorDays, setMaxTenorDays] = useState(initialPolicy?.maxTenorDays?.toString() || "60")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: BusinessPolicyPayload = {
      interestType,
      interestRate: parseFloat(interestRate) || 0,
      graceDays: parseInt(graceDays) || 0,
      maxTenorDays: parseInt(maxTenorDays) || 1,
      lateFeeFixed: lateFeeFixed ? parseFloat(lateFeeFixed) : undefined,
      lateFeeRate: lateFeeRate ? parseFloat(lateFeeRate) : undefined,
    }

    const result = await upsertBusinessPolicy(businessSlug, payload)
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                interestType === "FLAT" ? "bg-cyan-500/20" : "bg-white/10"
              }`}>
                <svg className={`w-5 h-5 ${interestType === "FLAT" ? "text-cyan-400" : "text-slate-400"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          >
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                interestType === "MONTHLY" ? "bg-blue-500/20" : "bg-white/10"
              }`}>
                <svg className={`w-5 h-5 ${interestType === "MONTHLY" ? "text-blue-400" : "text-slate-400"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <input
              ...
              required
              className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500/50 focus:ring-2 focus:ring-cyan-500/20 transition-all pr-16"
              placeholder="3"
            />
      ...
      <div className="flex items-center justify-end gap-4 pt-6 border-t border-white/5">
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-cyan-600 to-blue-600 text-white font-medium text-sm shadow-lg shadow-cyan-500/25 hover:shadow-cyan-500/40 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center gap-2"
        >
          {isLoading ? (
            <>
